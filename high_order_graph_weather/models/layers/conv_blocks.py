...
from torch_geometric.nn import MetaLayer, GCNConv
...
def build_graph_processor_block(
    ...
    return MetaLayer(
        ...
        # ),
        node_model=GCNConv(in_dim_node, hidden_dim_node)
    )
...
class ConvProcessor(nn.Module):
    ...
    def __init__(
        ...
        self.blocks.append(GCNConv(in_dim_node, hidden_dim_node))
        for _ in range(mp_iterations - 1):
            self.blocks.append(
                GCNConv(in_dim_node, hidden_dim_node)
